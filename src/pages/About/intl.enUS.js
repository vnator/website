const about_enUS = {
  title: 'About Us',
  profile: {
    title: 'Profile',
    paragraph_1:
      "We like to define ourselves as 'Software Crafters', not wanting to appear arrogant, maybe pretentious but the intention is only to expose our concern with details in the work.",
    paragraph_2:
      'We will do our best to analyze, design and develop the best possible way, focusing on the details, environment, infrastructure, standards and good practices, testing and maintenance. I can still add that we will do all this in a transparent and organized way demonstrating commitment to work.',
  },
  focus: {
    title: 'Foco',
    paragraph_1:
      "We develop web applications that prioritize customer needs and your product as a main goal. We will always seek results by analyzing two criteria, the first is: 'what is the best way to do something', and the second is: 'what is the resource that we have to develop this something' (resource, time, investment, workmanship or knowledge). No waste, impossible jobs that do not return a viable product, much less sloppy work with dead solutions.",
    paragraph_2:
      'If you are interested in knowing more about our stack of technology, please click',
    link: 'here',
  },
  values: {
    title: 'Values',
    paragraph:
      'For us, work is a quest, we will take advantage each moment and we will do everything its possible to reach our destination. With this purpose we have four pillars that support our decisions, our disposition and inspiration for the journey.',
    quest: {
      title: 'Quest',
      paragraph:
        'The Quest is the thing that inspires us, the emotion of the challenge, the excitement of discovery, and the will to create and transform stuff. We are ever Hungry for working with new technologies or applications, searching for new chellenges and new ways to overcome them, we travel the road because we like adventure.',
    },
    onus: {
      title: 'Onus',
      paragraph:
        'The Responsibility is what drives us forward with caution, keeps us firm in our decisions as difficult as they are. New technologies motivate us but always prepare ourselves before any jorney, making prudence decisions, choosing paths that can take us to the final goal with the resource that we have.',
    },
    experience: {
      title: 'Experience',
      paragraph:
        'Perhaps a journey take us the final award, but every journey brings us experience, so we value the experience. We always seek to obtain it, whether by practice or theory, we do not despise its value whether it is obtained by work, study or observation.',
    },
    faith: {
      title: 'Faith',
      paragraph:
        "Faith is what keeps us moving forward whichever of the difficulty, we believe that the important thing is honesty. Even if this is the hard way, let us follow if it is the right way, we may not understand God's plans, but we understand his love, so we remain faithful in Jesus Christ",
    },
  },
};

export { about_enUS };
